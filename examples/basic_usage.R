...
plot(model$scores,
     col = ifelse(seq_along(model$scores) %in% outliers, "red", "blue"),
     pch = 19, main = "Anomaly Scores", ylab = "Score",
     xlab = "Sample Index")
...
barplot(contrib, las = 2, col = "steelblue",
        main = paste("样本", most_anomalous, "的特征贡�?),
        ylab = "贡献分数")
...
cat("\n性能指标:\n")
cat("准确�?", round((TP + TN) / sum(confusion), 3), "\n")
cat("精确�?", round(precision, 3), "\n")
cat("召回�?", round(recall, 3), "\n")
cat("F1分数:", round(f1, 3), "\n")
...
for (thresh in thresholds) {
  ...
  cat("阈�?, thresh, ":", length(outliers), "个异常\n")
}
