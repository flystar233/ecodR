...
# 异常检测测试数据生成器
# ========================================
# 使用方法：在R中运行 source("examples/generate_anomaly_data.R")
...
generate_anomaly_test_data <- function(
  ...
  # 将15个特征分成4个尺度组
  scale_groups <- list(
    small = 1:4,      # 特征1-4: 小尺度 (0-10)
    medium = 5:8,     # 特征5-8: 中尺度 (0-100)
    large = 9:12,     # 特征9-12: 大尺度 (0-1000)
  ...
  # 为每个尺度组设置参数
  # 小尺度组 (0-10)
  means[scale_groups$small] <- runif(length(scale_groups$small), min = 2, max = 8)
  sds[scale_groups$small] <- runif(length(scale_groups$small), min = 0.5, max = 1.5)
  
  # 中尺度组 (0-100)
  means[scale_groups$medium] <- runif(length(scale_groups$medium), min = 30, max = 70)
  sds[scale_groups$medium] <- runif(length(scale_groups$medium), min = 5, max = 15)
  
  # 大尺度组 (0-1000)
  means[scale_groups$large] <- runif(length(scale_groups$large), min = 300, max = 700)
  sds[scale_groups$large] <- runif(length(scale_groups$large), min = 50, max = 150)
  
  # 超大尺度组 (0-10000)
  means[scale_groups$xlarge] <- runif(length(scale_groups$xlarge), min = 3000, max = 7000)
  sds[scale_groups$xlarge] <- runif(length(scale_groups$xlarge), min = 500, max = 1500)
  ...
  cat("特征数:", n_features, "\n")
  cat("正常样本数:", n_normal, "\n")
  cat("异常样本数:", n_outliers, "\n")
  cat("异常比例:", outlier_fraction * 100, "%\n")
  cat("========================================\n")
  cat("特征尺度分布:\n")
  cat("- Feature_1-4:   小尺度 (0-10)\n")
  cat("- Feature_5-8:   中尺度 (0-100)\n")
  cat("- Feature_9-12:  大尺度 (0-1000)\n")
  cat("- Feature_13-15: 超大尺度 (0-10000)\n")
  cat("========================================\n")
  cat("异常数据所在行号（共", length(outlier_rows), "行）:\n")
