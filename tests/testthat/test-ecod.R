...
test_that("ecod input validation works", {
  expect_error(ecod("not a matrix"), "'data' must be a matrix or data frame")
  expect_error(ecod(matrix(letters[1:20], ncol = 2)), "must contain only numeric values")
  expect_error(ecod(matrix(1:5, nrow = 1)), "must have at least 2 samples")
})
